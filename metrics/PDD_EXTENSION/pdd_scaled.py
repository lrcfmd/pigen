...
from pdd import _nearest_neighbours as nearest_neighbors

# Chemistry same/cross species
from pdd_chemistry_same_cross import pdd_split 
...
def PDD_scale(periodic_set, k=30):
    """
    Computes the scaled Pointwise Distance Distribution (PDD) of a periodic set,
    with distances scaled by number density (n / V).

    Parameters
    ----------
    periodic_set : PeriodicSet
        The periodic set to compute the scaled PDD for.
    k : int
    ...
    -------
    np.ndarray
        The scaled PDD matrix: one row per point (weight + k distances).
    """
    from pdd import pdd as PDD

    # Compute the original PDD
    pdd = PDD(periodic_set, k=k)

    # Access the motif and lattice matrix
    motif = periodic_set.motif   # numpy array of shape (n_points, 3)
    lattice = periodic_set.cell  # numpy array of shape (3, 3)

    # Compute number of points
    n_points = motif.shape[0]

    # Compute unit cell volume
    volume = abs(np.linalg.det(lattice))

    # Calculate number density
    scale = (n_points / volume) ** (1/3)

    # Scale the distances (skip the first column, which is the weight)
    pdd_scaled = pdd.copy()
    pdd_scaled[:, 1:] *= scale

    return pdd_scaled

def PDD_scale_split(periodic_set, k=30, **pdd_split_kwargs):
    """
    Computes the scaled same/cross-species split PDD (experiment 2f),
    with distances scaled by the linear number density scale (n/V)^(1/3).

    Parameters
    ----------
    periodic_set     : PeriodicSet with a `.types` tag
    k                : number of nearest neighbours
    **pdd_split_kwargs : passed through to pdd_split (collapse, lexsort, etc.)

    Returns
    -------
    pdd_same_scaled  : np.ndarray, shape (m, k+1)
    pdd_cross_scaled : np.ndarray, shape (m, k+1)
    """
    pdd_same, pdd_cross = pdd_split(periodic_set, k=k, **pdd_split_kwargs)

    motif   = periodic_set.motif
    lattice = periodic_set.cell
    scale   = (motif.shape[0] / abs(np.linalg.det(lattice))) ** (1/3)

    pdd_same_scaled  = pdd_same.copy();  pdd_same_scaled[:, 1:]  *= scale
    pdd_cross_scaled = pdd_cross.copy(); pdd_cross_scaled[:, 1:] *= scale

    return pdd_same_scaled, pdd_cross_scaled
...
def PDD_reconstructable(pset: PeriodicSet, lexsort: bool = True) -> FloatArray:
    ...
    pdd, _, _ = nearest_neighbors(pset.motif, pset.cell, min_val) #-- wrong
    if lexsort:
