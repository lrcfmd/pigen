...
df_cs  = torch.tensor(np.cumsum(df_amds, axis=1), dtype=torch.float32, device=device)  # (N_ref,   k)
print(df_cs.shape)

#bfs = ['/home/u6fo/andrij.u6fo/pigen/generated/PDDMIN/gen_PDDMIN3/denovo_[3.0]_2.0_compact5141_spp.csv_AMD.pickle',
#       '/home/u6fo/andrij.u6fo/pigen/generated/PDDMIN/gen_PDDMIN4/denovo_[4.0]_2.0_compact4519_spp.csv_AMD.pickle',
#       '/home/u6fo/andrij.u6fo/pigen/generated/PDDMIN/gen_PDDMIN5/denovo_[5.0]_2.0_compact3815_spp.csv_AMD.pickle']
...
for bfile in bfs:
    ...
    # Precompute cumsums on GPU
    print('Precompute cumsums on GPU for bfile')
    bf_cs  = torch.tensor(np.cumsum(bf_amds, axis=1), dtype=torch.float32, device=device)  # (N_probe, k)

    N_probe = bf_cs.shape[0]
    N_ref   = df_cs.shape[0]

    PDDMIN      = np.full(N_probe, np.inf)   # min distance to any reference
    PDDMIN_idx  = np.zeros(N_probe, dtype=int)  # index of nearest reference

    BLOCK_REF = 5000  # tune to VRAM; reduce if OOM

    for j in tqdm(range(0, N_ref, BLOCK_REF)):
        ref_chunk = df_cs[j:j+BLOCK_REF]
        D_block = (bf_cs.unsqueeze(1) - ref_chunk.unsqueeze(0)).abs().sum(-1)
    ...
    bf1['PDDMIN']     = PDDMIN
    bf1['PDDMIN_ref'] = df.iloc[PDDMIN_idx].index.values  # or a name column if preferred
    
    bf1.to_csv(f'{bfile}_PDDMIN.csv', index=False)
