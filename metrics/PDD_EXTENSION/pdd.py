...
def read_cif(path):
    ...
    for block in ase.io.cif.parse_cif(path):
        ...
        #print(frac_motif, asym_inds, multiplicities, inverses)
...
def read_cif_pmg(structure):
    cif_str = str(CifWriter(structure))
    parser = CifParser(io.StringIO(cif_str)) # path == str
    #parser = CifParser(io.StringIO(structure)) # path == str
     
    result = []
    #structure = parser.get_structures(primitive=False)[0]
    #by blocks:
    cif_dict = parser.as_dict()
    block = list(cif_dict.values())[0]
    x = block['_atom_site_fract_x']
    y = block['_atom_site_fract_y']
    z = block['_atom_site_fract_z']
    asym_unit = np.mod(np.array([x, y, z], dtype=float).T, 1)
    labels = block['_atom_site_type_symbol']
    asym_types = [Element(sym).Z for sym in labels]

    if '_symmetry_equiv_pos_as_xyz' in block:
        sitesym = block['_symmetry_equiv_pos_as_xyz']
    elif '_space_group_symop_operation_xyz' in block:
        sitesym = block['_space_group_symop_operation_xyz']
    else:
       raise ValueError("No symmetry operations found in CIF.")
    ...
    try:
        ...
        print("Couldnt _expand_asym_unit --- I dont' have frac_motif as a result or multiplicities")
    full_types = np.array([asym_types[i] for i in inverses])
    motif = frac_motif @ cell
...
#   full_coords = sym_struct.frac_coords
#   try:
#       full_types_ = [site.specie.number for site in sym_struct]
#   except:
#       full_types_ = [site.species.number for site in sym_struct]
#
#   equivalent_groups = sym_struct.equivalent_sites
#   #print(equivalent_groups)
#   try:
#       asym_inds_ = [group.index for group in equivalent_groups]
#   except:
#       print('Cannot run [group.index for group in equivalent_groups] to get asym_inds_')
#   multiplicities_ = [len(group) for group in equivalent_groups]
#   if  np.any(np.mod(multiplicities_, 1) != 0) or np.any(np.mod(multiplicities, 1) != 0):
#       print(multiplicities)
#       print(multiplicities_)

    #assert(asym_inds == asym_inds_)
    #assert(multiplicities == multiplicities_)
    #assert(full_types == full_types_)
    ...
    tags = {
        ...
        'wyckoff_multiplicities': multiplicities,
        'types': full_types,
        'cellpar': structure.lattice.parameters,
        #'wyckoff_symbols': sym_struct.wyckoff_symbols,
    }
    
    result.append(PeriodicSet(motif, cell, **tags))
