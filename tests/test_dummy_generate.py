...
from pigen.settings import PROJECT_ROOT, CHECKPOINT_DIR, config

RECOMMENDED_STEP_LR = {'gen': {
                              'full_data':5e-6}
                              }

CKPT_PATH = os.path.join(CHECKPOINT_DIR, 'pigen_CE_step215150.ckpt')

pytestmark = pytest.mark.skipif(
    not CKPT_PATH.is_file(),
    reason="ckpt.ckpt checkpoint file not found; skipping generate tests"
)
...
def generate(
    ...
    with open(model_path / 'dummy_settings.yaml', 'r') as f:
    ...
    for i, structure in enumerate(struct_list):
        ...
        if structure is not None:
            structure.to(f'{save_path}/{formula}.cif', fmt='cif')
...
def test_dummy_generate():
    this_dir = Path(__file__).parent
    dummy_dir = CHECKPOINT_DIR 
    dummy_save = this_dir / 'dummy_logs' / 'dummy_generation_run'
    generate(model_path=dummy_dir, save_path=dummy_save, 
             dataset='full_data', step_lr=5e-6, num_batches_to_samples=1,
             guidance=2, targets=[9, 0.7], batch_size=3)
