...
def calculate_atomic_volume(element: str, charge=False) -> float:
    """
    ...
    try:
        if charge:
            if element in ['O', 'S', 'F', 'Cl', 'Br', 'I', 'Te', 'Se']:
                radius = Element(element).average_anionic_radius
            else:
                radius = Element(element).average_cationic_radius
        else:
...
def calculate_compactness(structure: Structure, charge=False) -> float:
    """
    ...
    try:
        ...
        total_atomic_volume = sum(
            calculate_atomic_volume(element, charge) * count
...
def main():
    ...
    parser.add_argument(
            '--charge',
            action='store_true',
            help='Use average cation and anion radii, based on whether element is O, S, F, Cl, Br, I, Se, Te'
            )
    ...
    try:
        if args.no_filter:
            ...
            for cif_string in df[args.cif_column]:
                try:
                    structure = Structure.from_str(cif_string, fmt='cif')
                    compactness_values.append(calculate_compactness(structure, args.charge))
                except Exception:
                    compactness_values.append(np.nan)
            df_result['compactness'] = compactness_values
            df_filtered = df_result
