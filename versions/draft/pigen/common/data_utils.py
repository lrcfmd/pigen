...
def build_crystal(crystal_str, niggli=True, primitive=False):
    ...
    for site in crystal.sites:
        if len(site.species.as_dict().items()) >1:
            print('Disordered struct:', site.species)
            break
...
def get_scalers_from_data_list(data_list, key):
    scalers = []
...
def get_scaler_from_data_list(data_list, key):
    targets = torch.tensor(np.array([d[key] for d in data_list],dtype=np.float32), dtype=torch.float32)
    scaler = StandardScalerTorch()
    scaler.fit(targets)
    return scaler
...
def preprocess_simple(input_file, num_workers, niggli, primitive,prop_list, 
    ...
    def process_one_simple(row, niggli, primitive, prop_list, use_space_group = False, tol=0.01, target_energy=target_energy):
        ...
        frac_coords = crystal.frac_coords
        atom_types = np.array(crystal.atomic_numbers)
        lattice_parameters = crystal.lattice.parameters
        lengths = np.array(lattice_parameters[:3])
        angles = np.array(lattice_parameters[3:])
        num_atoms = atom_types.shape[0]

        crys_arrays = (frac_coords, atom_types, lengths, angles, num_atoms)
    ...
    # prop = prop_list[0]
    if prop_list != ['']:
        df   = df.dropna(subset=prop_list).reset_index(drop=True)
...
def preprocess(input_file, num_workers, niggli, primitive, graph_method,
    ...
    prop = prop_list[0]
    df   = df.dropna(subset=[prop]).reset_index(drop=True)
...
def preprocess_tensors(crystal_array_list, niggli, primitive, graph_method):
    ...
    #adding this line temporarily to check fine-tuned bandgap
...
def add_scaled_lattice_prop(data_list, lattice_scale_method):
    for dict in data_list:
        crys_arrays = dict['crys_arrays']
        # the indexes are brittle if more objects are returned
        lengths = crys_arrays[2]
        angles = crys_arrays[3]
        num_atoms = crys_arrays[-1]
