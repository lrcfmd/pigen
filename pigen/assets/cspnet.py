...
from pigen.common.data_utils import lattice_params_to_matrix_torch, get_pbc_distances
from pigen.common.data_utils import radius_graph_pbc, frac_to_cart_coords, repeat_blocks
...
def build_f_emb(prop_dim=1, n_props=2, hidden_dim=512):
...
class CSPNet(nn.Module):
    def __init__(
        self, hidden_dim = 512, latent_dim = 0, num_layers = 6, max_atoms = 100,
        act_fn = 'silu', dis_emb = 'sin', num_freqs = 128, edge_style = 'fc',
        cutoff = 7.0, max_neighbors = 20, ln = False, ip = True,
        smooth = False, pred_type = True, pred_scalar = False, 
        n_props: int = 2, prop_weights = [1.,1.], p_cond = 0.4):
        ...
        if self.pred_type:
            self.type_out = nn.Linear(hidden_dim, MAX_ATOMIC_NUM) #modified to 2* to try concat
        ...
        if self.n_props > 0:
            self.cond_emb     = build_f_emb(prop_dim=1, n_props=n_props, hidden_dim=hidden_dim)
            self.p_cond       = p_cond
    ...
    def forward(self, t, atom_types, frac_coords, lattices, num_atoms, 
                node2graph, condition=None, cond_weights=None, stage='train'):
        ...
        if condition is not None:
            split_conditions = condition.split(1, dim=1)  # This will give a list of (batch_size, 1) tensors
            if stage == 'train': #train stage is probabilistic
                p = random.random()
                if p > 1 - self.p_cond:
                    for cond, w, nn in zip(split_conditions, self.prop_weights, self.cond_emb):
                        cond_emb = nn(cond)
                        cond_emb = cond_emb.repeat_interleave(num_atoms, dim=0)
                        node_features+= w*cond_emb
            elif stage == 'eval':
                for cond, nn in zip(split_conditions, self.cond_emb):
                    cond_emb = nn(cond)
                    cond_emb = cond_emb.repeat_interleave(num_atoms, dim=0)
                    node_features+=cond_emb
