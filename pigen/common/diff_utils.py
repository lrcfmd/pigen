...
def sample_norm(sigma, T=1.0, sn = 10000, num_atoms=52):
    ...
    # for normalization
    #sample_norm = torch.load(Path(__file__).parent.parent / 'normalization' / 'sample_norm.pth').to('cpu')
    sample_norm = torch.load(Path(__file__).parent.parent / 'normalization' / 'sample_norm.pth', weights_only=False, map_location=torch.device('cpu'))
    return sample_norm

def kappa_func():
    # created by 'equicsp/pl_modules/von_mises_norm.py'
    # for normalization
    #kappa_matrix = torch.load(Path(__file__).parent.parent / 'normalization' / 'kappa_matrix.pth', weights_only=False)
    #kappa_norm = torch.load(Path(__file__).parent.parent / 'normalization' / 'kappa_norm.pth', weights_only=False)
    kappa_matrix = torch.load(Path(__file__).parent.parent / 'normalization' / 'kappa_matrix.pth', weights_only=False,  map_location=torch.device('cpu'))
    kappa_norm = torch.load(Path(__file__).parent.parent / 'normalization' / 'kappa_norm.pth', weights_only=False, map_location=torch.device('cpu'))
